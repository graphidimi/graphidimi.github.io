...
Glyph.prototype.drawPoints = function(ctx, x, y, fontSize) {
    ...
    function drawCircles(l, x, y, scale) {
        ...
        for (var j = 0; j < l.length; j += 1) {
			...
			//ctx.fillRect(x + (l[j].x * scale), y + (l[j].y * scale), 20, 20);
			//ctx.drawImage(image, x + (l[j].x * scale), y + (l[j].y * scale), 20, 20);
			ctx.globalAlpha = 0.8;
			ctx.drawImage(document.images[j], x + (l[j].x * scale)-5, y + (l[j].y * scale)-5, 25, 25);
        }
